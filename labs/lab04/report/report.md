...
Установил git-flow (см. [рис.1](#fig-001)).

![Установка git-flow](image/1.png){#fig-001 width=70%}

### Установка Node.js

Установил Node.js (см. [рис.2](#fig-002)).

![Установка Node.js](image/2.png){#fig-002 width=70%}

### Настройка Node.js

Запустил `pnpm setup` и выполнил `source ~/.bashrc` (см. [рис.3](#fig-003)).
...
- Создал репозиторий на GitHub. Назвал его `git-extended` (см. [рис.4](#fig-004)).

![Создание репозитория GitHub](image/4.png){#fig-004 width=70%}

- Сделал первый коммит и выложил его на github (см. [рис.5](#fig-005)).
...
Заполнил несколько параметров пакета и сконфигурировал формат коммитов (см. [рис.7](#fig-007)).

![Заполнение нескольких параметров пакета и конфигурация формата коммитов](image/7.png){#fig-007 width=70%}

- Добавил новые файлы (см. [рис.8](#fig-008)).

![Добавление новых файлов](image/8.png){#fig-008 width=70%}

- Выполнил коммит (см. [рис.9](#fig-009)).

![Выполнение коммита](image/9.png){#fig-009 width=70%}

- Отправил на github (см. [рис.10](#fig-010)).

![Загрузка на github](image/10.png){#fig-010 width=70%}
...
- Проверил, что я на ветке `develop`, загрузил весь репозиторий в хранилище, установил внешнюю ветку как вышестоящую для этой ветки и создал релиз с версией 1.0.0 (см. [рис.12](#fig-012)).
...
Создал журнал изменений и добавил его в индекс (см. [рис.13](#fig-013)).

![Создание журнала изменений и добавление его в индекс](image/13.png){#fig-013 width=70%}

Залил релизную ветку в основную ветку, отправил данные и создал релиз на github (см. [рис.14](#fig-014)).

![Залил релизную ветку в основную ветку, отправка данных и создание релиза на github](image/14.png){#fig-014 width=70%}

### Работа с репозиторием git

1. Разработка новой функциональности

Создал ветку для новой функциональности и объединил ветку `feature_branch` с `develop` (см. [рис.15](#fig-015)).

![Создание ветки для новой функциональности и объединение ветки `feature_branch` с `develop`](image/15.png){#fig-015 width=70%}

2. Создание релиза git-flow

- Создал релиз с версией `1.2.3`, обновил номер версии в `package.json`, создал журнал изменений и добавил журнал изменений в индекс (см. [рис.16](#fig-016)).
...
- Залил релизную ветку в основную ветку, отправил данные на github, создал релиз на github с комментариями из журнала изменений (см. [рис.17](#fig-017)).

![Залил релизную ветку в основную ветку, отправка данных на github, создание релиза на github с комментариями из журнала изменений](image/17.png){#fig-017 width=70%}

## Преобразование рабочего репозитория в репозиторий с git-flow и conventional commits

Зашёл в папку `os-intro`, создал новый репозиторий, добавил все файлы в индекс, сохранил первый коммит и создал `package.json` (см. [рис.18](#fig-018)).

![Переход в папку os-intro, создание нового репозитория, добавление всех файлов в индекс, сохранение первого коммита и создание package.json](image/18.png){#fig-018 width=70%}

Заполнил несколько параметров пакета и сконфигурировал формат коммитов (см. [рис.19](#fig-019)).

![Заполнение параметров пакета и конфигурация формата коммитов](image/19.png){#fig-019 width=70%}

Сделал `git flow init`, настроил `master` и `develop`, префиксы веток и префикс тегов v. Проверил, что все ветки появились и запушил всё на GitHub. (см. [рис.20](#fig-020)).

![Выполнение git flow init, настройка master и develop, префиксов веток и префиксв тегов v. Проверка, появления веток и отправка на GitHub](image/20.png){#fig-020 width=70%}
